...
var handler = async (m, {
 ...
 command
    ...
    try {
        ...
        const res = `https://api.betabotz.eu.org/api/sticker/dinokuning?apikey=${global.lann}`
    ...
    if (stiker) await conn.sendFile(m.chat, stiker, '', author, m, null)
        ...
        const res = `https://api.betabotz.eu.org/api/sticker/paimon?apikey=${global.lann}`
            let stiker = await sticker(false, res, stickpack, stickauth)
    if (stiker) await conn.sendFile(m.chat, stiker, '', author, m, null)
        }
        else if (command == 'spongebob' || command == 'sspongebob') {
        const res = `https://api.betabotz.eu.org/api/sticker/spongebob?apikey=${global.lann}`
            let stiker = await sticker(false, res, stickpack, stickauth)
    if (stiker) await conn.sendFile(m.chat, stiker, '', author, m, null)
        }
        else if (command == 'manusialidi' || command == 'smanusialidi') {
        const res = `https://api.betabotz.eu.org/api/sticker/manusialidi?apikey=${global.lann}`
            let stiker = await sticker(false, res, stickpack, stickauth)
    if (stiker) await conn.sendFile(m.chat, stiker, '', author, m, null)
        }
    } catch (e) {
